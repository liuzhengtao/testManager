...
import org.yumin.zengDaWebUI.utils.*;
...
public class LoginTest extends TestBaseCase {
	...
	{
		//调用登录方法，输入正确的用户名和密码
		LoginAction loginAction=new LoginAction(BaseUrl, "kf1uat","Zd123456");
		action.sleep(2);
		//设置检查点
		Assertion.VerityTextPresentPrecision("管理系统","输入正确的用户名和密码，验证是否成功进入主页");
		//设置用例断言，判断用例是否失败
